 ...
 * 
 * 테스트 네이밍 규칙 
 * 	: [SQL 작업 내용]_[테스트 내용(Mapper 클래스 메소드 이름을 사용했음)]_[테스트 결과]
 * 
 * @Transactional
 * 	: 트랜잭션 테스트 후 트랜잭션을 강제 롤백해서 DB에 반영되지 않도록 한다.
 *
 */
...
public class UserMapperTest {
	...
	@Test
	public void insert_signUpUser_insertNewUser() {
		userMapper.signUpUser(user);
	}
	
	@Test
	public void select_isExistUserId_EXISTED() {
		userMapper.signUpUser(user);
		assertEquals("EXISTED", userMapper.isExistUserId(user.getUserId()));
	}
	
	@Test
	public void select_isExistUserId_NOT_EXISTED() {
		assertEquals("NOT_EXISTED", userMapper.isExistUserId(user.getUserId()));
	}
	
	@Test
	public void select_getUserById_showUserInfo() {
		userMapper.signUpUser(user);
		userMapper.getUserById(user.getUserId());
	}
	
	@Test
	public void select_getIdByNameAndPhone_showUserId() {
		userMapper.signUpUser(user);
		assertEquals(TEST_VALUE, userMapper.getIdByNameAndPhone(user));
	}
	
	@Test
	public void select_getAccountId_showUserAccountId() {
		userMapper.signUpUser(user);
	...
	@Test
	public void update_changeUserProfile_changeUserProfile() {
		userMapper.signUpUser(user);
		// 삽입한 유저 정보 출력
		UserVO insertUser = userMapper.getUserById(user.getUserId());
		// 유저 정보 수정
